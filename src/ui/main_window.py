...
def _nav_sections():
    return [
        ("outputs",   i18n.t("nav.outputs"),   "outputs",   False),
        ("inputs",    i18n.t("nav.inputs"),     "inputs",    True),
        ("playback",  i18n.t("nav.playback"),   "playback",  False),
        ("recording", i18n.t("nav.recording"),  "recording", True),
        ("streaming", i18n.t("nav.streaming"),  "streaming", None),
    ]


class NavButton(QPushButton):
    def __init__(self, icon_name: str, label: str, accent_variant: str):
        super().__init__()
        ...
        Icons.apply_to_button(self, icon_name, label, color=theme.TEXT_DIM)
...
class MainWindow(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        self.header_title = QLabel(i18n.t("main.header_title"))
        ...
        for key, label, icon_name, is_input in _nav_sections():
    ...
    def _build_sidebar(self):
        ...
        for key, label, icon_name, is_input in _nav_sections():
            ...
            accent_variant = (
                "nav-green" if is_input is None
                else ("nav-cyan" if is_input else "nav-amber")
            )
            button = NavButton(icon_name, label, accent_variant)
        ...
        Icons.apply_to_button(settings_btn, "settings", i18n.t("main.sidebar_settings"), color=theme.TEXT_DIM)
        layout.addWidget(settings_btn)

        changelog_btn = QPushButton()
        changelog_btn.setObjectName("sidebarChangelogButton")
        changelog_btn.setProperty("variant", "sidebar-action")
        changelog_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        changelog_btn.clicked.connect(self._open_changelog)
        Icons.apply_to_button(changelog_btn, "changelog", i18n.t("settings.view_changelog"), color=theme.TEXT_DIM)
        ...
        Icons.apply_to_button(refresh_btn, "refresh", i18n.t("main.sidebar_refresh"), color=theme.TEXT_DIM)
    ...
    def _fill_devices(self, key: str, dtype: DeviceType, is_input: bool):
        tab = self._pages[key]
        ...
        if not is_input:
            ...
            from src.core.streaming import MONITOR_SINK_NAME, STREAM_SINK_NAME
            internal_sink_names = channel_sink_names | {MONITOR_SINK_NAME, STREAM_SINK_NAME}
            devices = [d for d in devices if d.name not in internal_sink_names]
        ...
        self._sync_list(
            ...
            empty_text=i18n.t("main.no_device"),
        )

    def _fill_streams(self, key: str, stype: DeviceType, target_dtype: DeviceType):
        tab = self._pages[key]
        ...
        devices = [
            d
            for d in self.audio.get_devices(target_dtype)
            if not d.name.endswith(".monitor")
        ]
    ...
    def _apply_saved_routes(self, stype, streams, devices, is_input, streaming_enabled):
        ...
        current_indices = {s.index for s in streams}
    ...
    def _sync_list(self, tab, items, key, make, update, empty_text):
        ...
        for pos, item in enumerate(items):
            ...
            if item_key in tab.cards:
                ...
                update(card, item)
                placeholder_offset = 1 if tab.placeholder is not None else 0
                if layout.indexOf(card) != pos + placeholder_offset:
